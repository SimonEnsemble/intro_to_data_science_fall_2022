...
construct a `DataFrame`, `cities_data`, that contains information about cities in the US.
...
# cities_data = DataFrame(city=city, population=population)
cities_data = DataFrame(city=["Corvallis", "Portland", "Eugene"],
	                    population=[57961, 647805, 168916])

# ╔═╡ bc58ac1c-033c-11eb-2698-f5fc4c20b8ce
md"### append rows

append two new rows to `cities_data`:
...
begin
	...
	push!(cities_data, another_new_row)
end

# ╔═╡ 5e293c82-033d-11eb-3984-7164bf9a351d
md"### append columns

append another two column to `cities_data` giving the (1) annual rainfall in each city and (2) state to which the city belongs.
...
cities_data[:, "rain"] = [51.0, 43.0, 47.0, 12.0, 25.0]
...
insertcols!(cities_data, 2, :state => ["OR", "OR", "OR", "OR", "CA"])
...
rename!(cities_data, "rain" => "rainfall")
...
select!(cities_data, Not("twice_the_rainfall"))

# ╔═╡ 48d7d749-c7d0-407c-8863-8ab7530d6c03
select(cities_data, ["city", "state"]) # no !, hence not modified.
...
push!(cities_data, ["bogus", "blah", 0, 100.0])

# ╔═╡ b7fb0648-0390-11eb-2dc5-8b6935d2545c
delete!(cities_data, 6)
...
push!(cities_data, ["Berkeley", "CA", 122324, 25.0]) 
...
filter(row -> row["city"] == "Corvallis", cities_data)
...
sort!(cities_data, "population", rev=true)
...
e.g., group the cities (rows) in `cities_data` by state.
"

# ╔═╡ c1526020-035d-11eb-2d8a-d131aa445738
gb_state = groupby(cities_data, :state)
...
df_rain = combine(groupby(cities_data, "state"), "rainfall" => mean => "avg_rainfall")
